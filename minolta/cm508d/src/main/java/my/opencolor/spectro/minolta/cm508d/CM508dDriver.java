 ...
 */
public class CM508dDriver implements SpectroDriver
{
    static {
        DriverManager manager = DriverManager.getInstance();
        try
        {
            manager.registerDriver(CM508dDriver.class);
        }
        catch ( my.opencolor.spectro.spi.SpectroException ex)
        {
            System.err.println("Unable to registed CM508d Driver");
    ...
    public void initialize()
    {
        ...
        m_LensPositions = new LensPosition[] { new StandardAreaView()};

        m_Apertures = new Aperture[] { new StandardAperture()};
