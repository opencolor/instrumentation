...
import my.opencolor.spectro.spi.helpers.*;
    ...
    implements Spectrophotometer, CommDriverListener, Runnable
{
    ...
    static
    {
        m_Logger = Logger.getLogger( CM3600dSpectro.class.getName() );
    }
    ...
    //Serial number of the instrument, none for the CM3600d
    ...
    // Additional parameter for CM3600d
            ...
            switch( step )
            {
                ...
                case 1: //White Calibration
                    m_Outgoing.putObject(
                        new CPSCommand(
                            new Integer( 1 ),
                            new Boolean( m_Settings.getSpecular() ),
                            m_Settings.getLensPosition(),
                            new Integer( 999 ),
                            new Integer( 0 ) ) );
    ...
    public void setSettings( SpectroSettings newSettings )
    {
        synchronized( this )
        {
            m_Logger.info( "CM3600d Spectro : Set settings called" );

            if( newSettings == null )
            {
                m_Logger.info( "CM3600d Spectro : Null settings enterred" );
                return;
            }
            ...
            if( m_CommDriver == null )
            {
                ...
                try
                {
                    ...
                    m_Logger.info(
                        "CM3600d Spectro : Comm Settings complete... return" );
                    return;
                }
                catch( NumberFormatException numEx )
                {
                    ...
                    catch( CommDriverException commEx )
                    {
                        ...
                        //to open connection
                        m_Logger.info(
                            "CM3600d Spectro : FAILURE TO OPEN CONNECTION" );

                        SpectroStatus status = CM3600dStatus.create( "OPEN_FAILED" );
                        status.addMessage( "MSG_ERROR_OPENING_PORT" );
                        notifyStatusChange( new SpectroEvent( this, status ) );
                        m_Logger.info( "CM3600d Spectro : Comm Settings complete... return" );
                        return;
                    }
                ...
                catch( CommDriverException commEx )
                {
                    ...
                    m_Logger.info( "CM3600d Spectro : FAILURE TO OPEN CONNECTION" );

                    SpectroStatus status = CM3600dStatus.create( "ERROR_OPENING" );
                    status.addMessage( "MSG_ERROR_OPENING_PORT" );

                    notifyStatusChange( new SpectroEvent( this, status ) );

                    m_Logger.info( "CM3600d Spectro : Unable to open port... return" );
                    return;
                }

                m_Logger.info( "CM3600d Spectro : Should not reach this return in set settings" );
                return;
            }
            ...
            m_Logger.info( "CM3600d Spectro : Set setings done" );
        }
    }
    ...
    public void dispose()
    {
        m_Logger.info( "CM3600d Spectro : Dispose called" );
        ...
        if( m_CommDriver != null )
        {
            ...
            {
                m_Logger.info( "CM3600d Spectro : FAILURE TO CLOSE CONNECTION" );
            }
        }
        ...
        if( running )
        {
            m_Logger.info( "CM3600d Spectro : Thread still running" );
        }
    ...
    {
        m_Logger.info( "CM3600d Spectro : Interpreting command" );
        m_Logger.info( "CM3600d Spectro : " + message );
        ...
        try
        {
            if( !m_Incoming.isEmpty() )
            {
                ...
                m_Logger.info( "CM3600d Spectro : Interpreting > " + message );
                m_Logger.info( "CM3600d Spectro : Interpreting as a " + cmd.getName() + " command" );
                ...
                if( evt != null )
                {
                    ...
                    m_Logger.info( "CM3600d Spectro : Cancelling timeout at " + System.currentTimeMillis() );
                    ...
                    {
                        m_Logger.info( "CM3600d Spectro : Request Status Command" );
                        m_Incoming.removeNextObject();
                        m_Logger.info( "CM3600d Spectro : Removing Request Status Command from Incoming" );

                        m_ChargeStatus = ( (STRCommand) cmd ).getChargeStatus();
                        m_WhiteCalibration = ( (STRCommand) cmd ).getWhiteCalibration();
                    ...
                    {
                        m_Logger.info(
                            "CM3600d Spectro : Measurement Data Output Request from Incoming" );
                        m_Incoming.removeNextObject();
                        m_Logger.info( "CM3600d Spectro : Removing Measurement Data Output Request from Incoming" );
                    ...
                    {
                        m_Logger.info( "CM3600d Spectro : Measurement Parameters Prepare Command from Incoming" );
                        m_Incoming.removeNextObject();
                        m_Logger.info( "CM3600d Spectro : Removing Measurement Parameters Prepare Command from Incoming" );

                        return;
                    }
                    else if( cmd instanceof UZCCommand )
                    {
                        m_Logger.info( "CM3600d Spectro : User Zero Calibration Command from Incoming" );
                        m_Incoming.removeNextObject();
                        m_Logger.info( "CM3600d Spectro : Removing User Zero Calibration Command from Incoming" );
                        return;
                    }
                    else if( cmd instanceof CALCommand )
                    {
                        m_Logger.info( "CM3600d Spectro : White Calibration Command from Incoming" );
                        m_Incoming.removeNextObject();
                        m_Logger.info( "CM3600d Spectro : Removing White Calibration Command from Incoming" );
                        return;
                    }
                    else
                    {
        ...
        catch( SpectroException ep )
        {
            ...
            m_Logger.log( Level.SEVERE,  "Problem guessing the message.", ep );
        }
    }
    ...
    {
        m_Logger.info( "CM3600d Spectro : Received event from CommDriver" );

        //Indication of Data Available in the input stream
        if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_RECEIVED )
        {
            m_Logger.info( "CM3600d Spectro : Comm Driver Received" );
            ...
            m_Logger.info( "CM3600d Spectro : Before clearing > " + m_Received );
            ...
            m_Logger.info( "CM3600d Spectro : Buffer > " + m_Received );
        }
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_TIMEOUT )
        {
            ...
            SpectroStatus status = CM3600dStatus.create( "TIMEOUT_ERROR" );
            ...
            //Remove from object from the FIFO
            SpectroCommand cmd = (SpectroCommand) m_Incoming.removeNextObject();

            //m_Logger.info ( "CM3600d Spectro : Pre-empting " + cmd.getName () );

            m_Logger.info( "CM3600d Spectro : Timeout received for " + cmd.getName() );
            m_Logger.info( "CM3600d Spectro : Timeout received at " + System.currentTimeMillis() );
        ...
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_SENDING )
        {
            ...
            m_Logger.info( "CM3600d Spectro : Comm Driver Sending" );
        }
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_SENT )
        {
            //Notify sent command -- Output buffer/stream is empty
            m_Logger.info( "CM3600d Spectro : Comm Driver Sent" );
        }
        else
        {
            //Should not happen : Unknown comm status event
            m_Logger.info( "CM3600d Spectro : Unrecognized Comm Driver Event" );
        }
    }

    public void sent( CommDriverEvent evt )
    {
        m_Logger.info( "CM3600d Spectro : Sent event from CommDriver" );
    }

    public void notifyMeasured( SpectroEvent evt )
    {
        m_Logger.info( "CM3600d Spectro : Measurement received" );
    ...
    {
        m_Logger.info( "CM3600d Spectro : Calibration received" );
        Iterator list = m_Listeners.iterator();
    ...
    {
        m_Logger.info( "CM3600d Spectro : Settings Ack received" );
        Iterator list = m_Listeners.iterator();
    ...
    {
        m_Logger.info( "CM3600d Spectro : Status change " );
        Iterator list = m_Listeners.iterator();
    ...
    public void run()
    {
        while( running )
        {
            ...
            {
                m_Logger.info( "CM3600d Spectro : Thread still running" );
                m_Logger.info( "CM3600d Spectro : Retreiving Command from fifo" );

                //Retrieve the command
                SpectroCommand cmd = (SpectroCommand) m_Outgoing.getNextObject();

                m_Logger.info( "CM3600d Spectro : Constructing command" );
                String message = cmd.construct();

                message = message + "\r\n";

                m_Logger.info( "CM3600d Spectro : Message constucted > " + message );

                m_Logger.info( "CM3600d Spectro : Sending " + cmd.getName() + " at " + System.currentTimeMillis() );
                ...
                m_Logger.info( "CM3600d Spectro : Message posted." );
            }
            else
            {
                //Incoming FIFO is full, spectro is currently busy
                //m_Logger.info( "CM3600d Spectro : Spectro busy." );
            }
        }

        //Indicate that the thread has stopped running
        m_Logger.info( "CM3600d Spectro : Thread stopped." );
    }
    ...
    private SpectroSettings createSettings( String statusString )
    {
        ...
        {
            m_Logger.info ( "CM3600d Spectro : Specular set to Excluded" );
            m_Settings.setSpecular ( false );
        }
        else if ( statusString.charAt ( 3 ) == '0' )
        {
            m_Logger.info ( "CM3600d Spectro : Specular set to Included" );
            m_Settings.setSpecular ( true );
        }*/
