...
public class CM508cSpectro implements Spectrophotometer, CommDriverListener, Runnable
{
    static private Logger m_Logger = Logger.getLogger( CM508cSpectro.class.getName() );
    ...
    protected String m_SerialNo; //Serial number of the instrument, none for the CM508c
    ...
    // Additional parameter for CM508c

    public CM508cSpectro()
    {
    ...
    public void setSettings( SpectroSettings newSettings )
    {
        synchronized( this )
        {
            m_Logger.info( "CM508c Spectro : Set settings called" );

            if( newSettings == null )
            {
                m_Logger.info( "CM508c Spectro : Null settings enterred" );
                return;
            }
            ...
            if( m_CommDriver == null )
            {
                ...
                try
                {
                    ...
                    try
                    {
                        // Check whether is a Minolta CM508c or not
                    ...
                    m_Logger.info( "CM508c Spectro : Comm Settings complete... return" );
                    return;
                }
                catch( NumberFormatException numEx )
                {
                    ...
                    catch( CommDriverException commEx )
                    {
                        ...
                        //to open connection
                        m_Logger.info( "CM508c Spectro : FAILURE TO OPEN CONNECTION" );

                        SpectroStatus status = CM508cStatus.create( "OPEN_FAILED" );
                ...
                catch( CommDriverException commEx )
                {
                    ...
                    SpectroStatus status = CM508cStatus.create( "ERROR_OPENING" );
                    status.addMessage( "MSG_ERROR_OPENING_PORT" );

                    notifyStatusChange( new SpectroEvent( this, status ) );

                    m_Logger.info( "CM508c Spectro : Unable to open port... return" );
                    return;
                }

                m_Logger.info( "CM508c Spectro : Should not reach this return in set settings" );
                return;
            }
            ...
            m_Logger.info( "CM508c Spectro : Set setings done" );
        }
    }
    ...
    public void dispose()
    {
        m_Logger.info( "CM508c Spectro : Dispose called" );
        ...
        if( m_CommDriver != null )
        {
            ...
            {
                m_Logger.info( "CM508c Spectro : FAILURE TO CLOSE CONNECTION" );
            }
        }
        ...
        if( running )
        {
            m_Logger.info( "CM508c Spectro : Thread still running" );
        }
    ...
    {
        m_Logger.info( "CM508c Spectro : Interpreting command" );
        m_Logger.info( "CM508c Spectro : " + message );
        ...
        if( !m_Incoming.isEmpty() )
        {
            ...
            m_Logger.info( "CM508c Spectro : Interpreting > " + message );
            m_Logger.info( "CM508c Spectro : Interpreting as a " + cmd.getName() + " command" );
            ...
            if( evt != null )
            {
                ...
                m_Logger.info( "CM508c Spectro : Cancelling timeout at " + System.currentTimeMillis() );
                ...
                {
                    m_Logger.info( "CM508c Spectro : Measure Command" );
                    m_Incoming.removeNextObject();
                    m_Logger.info( "CM508c Spectro : Removing Measure Command from Incoming" );
                ...
                {
                    m_Logger.info( "CM508c Spectro : White Calibration Command" );
                    m_Incoming.removeNextObject();
                    m_Logger.info( "CM508c Spectro : Removing White Calibration Command from Incoming" );
                ...
                {
                    m_Logger.info( "CM508c Spectro : Instrument Identification Request Command" );
                    m_Incoming.removeNextObject();
                    m_Logger.info( "CM508c Spectro : Instrument Identification Request Command from Incoming" );
                ...
                {
                    m_Logger.info( "CM508c Spectro : Set Measurement Parameter Command" );
                    m_Incoming.removeNextObject();
                    m_Logger.info( "CM508c Spectro : Set Measurement Parameter Command from Incoming" );
    ...
    {
        m_Logger.info( "CM508c Spectro : Received event from CommDriver" );

        //Indication of Data Available in the input stream
        if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_RECEIVED )
        {
            m_Logger.info( "CM508c Spectro : Comm Driver Received" );
            ...
            m_Logger.info( "CM508c Spectro : Before clearing > " + m_Received );
            ...
            m_Logger.info( "CM508c Spectro : Buffer > " + m_Received );
        }
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_TIMEOUT )
        {
            ...
            SpectroStatus status = CM508cStatus.create( "TIMEOUT_ERROR" );
            ...
            //m_Logger.info ( "CM508c Spectro : Pre-empting " + cmd.getName () );

            m_Logger.info( "CM508c Spectro : Timeout received for " + cmd.getName() );
            m_Logger.info( "CM508c Spectro : Timeout received at " + System.currentTimeMillis() );
        ...
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_SENDING )
        {
            ...
            m_Logger.info( "CM508c Spectro : Comm Driver Sending" );
        }
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_SENT )
        {
            //Notify sent command -- Output buffer/stream is empty
            m_Logger.info( "CM508c Spectro : Comm Driver Sent" );
        }
        else
        {
            //Should not happen : Unknown comm status event
            m_Logger.info( "CM508c Spectro : Unrecognized Comm Driver Event" );
        }
    }

    public void sent( CommDriverEvent evt )
    {
        m_Logger.info( "CM508c Spectro : Sent event from CommDriver" );
    }

    public void notifyMeasured( SpectroEvent evt )
    {
        m_Logger.info( "CM508c Spectro : Measurement received" );
    ...
    {
        m_Logger.info( "CM508c Spectro : Calibration received" );
        Iterator list = m_Listeners.iterator();
    ...
    {
        m_Logger.info( "CM508c Spectro : Settings Ack received" );
        Iterator list = m_Listeners.iterator();
    ...
    {
        m_Logger.info( "CM508c Spectro : Status change " );
        Iterator list = m_Listeners.iterator();
    ...
    public void run()
    {
        while( running )
        {
            ...
            {
                m_Logger.info( "CM508c Spectro : Thread still running" );
                m_Logger.info( "CM508c Spectro : Retreiving Command from fifo" );

                //Retrieve the command
                SpectroCommand cmd = (SpectroCommand) m_Outgoing.getNextObject();

                m_Logger.info( "CM508c Spectro : Constructing command" );
                String message = cmd.construct();

                message = message + "\r\n";

                m_Logger.info( "CM508c Spectro : Message constucted > " + message );

                m_Logger.info( "CM508c Spectro : Sending " + cmd.getName() + " at " + System.currentTimeMillis() );
                ...
                m_Logger.info( "CM508c Spectro : Message posted." );
            }
            else
            {
                //Incoming FIFO is full, spectro is currently busy
                //m_Logger.info( "CM508c Spectro : Spectro busy." );
            }
        }

        //Indicate that the thread has stopped running
        m_Logger.info( "CM508c Spectro : Thread stopped." );
    }
    ...
    private SpectroSettings createSettings( String statusString )
    {
        ...
        {
            m_Logger.info ( "CM508c Spectro : Specular set to Excluded" );
            m_Settings.setSpecular ( false );
        }
        else if ( statusString.charAt ( 3 ) == '0' )
        {
            m_Logger.info ( "CM508c Spectro : Specular set to Included" );
            m_Settings.setSpecular ( true );
        }*/
