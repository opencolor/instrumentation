 ...
 */
public class SpectrolinoDriver implements SpectroDriver
{
    static {
        DriverManager manager = DriverManager.getInstance();
        try
        {
            manager.registerDriver(SpectrolinoDriver.class);
        }
        catch ( my.opencolor.spectro.spi.SpectroException ex)
        {
            System.err.println("Unable to register Spectrolino Driver ");
    ...
    public void initialize()
    {
        ...
        m_Apertures = new Aperture[] { new SmallAperture()};

        m_LightFilters = new LightFilter[] { new UVIncludedLightFilter(), new UVExcludedLightFilter(), new PolarizerLightFilter(), new D65LightFilter(), new CustomLightFilter()};

        m_LensPositions = new LensPosition[] { new SmallAreaView()};
    ...
    public String getLocalizedText(String text, Locale locale)
    {
        try
        {
            return ResourceBundle.getBundle("my.opencolor.spectro.drivers.spectrolino.Bundle", locale).getString(text);
        }
