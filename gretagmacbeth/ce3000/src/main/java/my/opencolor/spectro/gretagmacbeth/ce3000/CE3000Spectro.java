...
//                m_Startup = true;  will short circuit the use of m_Recovery
    ...
    private void modifySettings( String status )
    {
        ...
        if( status.charAt( 4 ) == 'S' && m_Settings.getLensPosition().getFocusRadius() != new SmallAreaView().getFocusRadius() )
        {
            m_Settings.setLensPosition( new SmallAreaView() );
            modified = true;
        }
        else if( status.charAt( 4 ) == 'L' && m_Settings.getLensPosition().getFocusRadius() != new LargeAreaView().getFocusRadius() )
        {
