...
public class UVFilterCommand
    implements SpectroCommand
{
    protected boolean m_UVincluded;

    /**
     * Creates a new instance of MeasureCommand
     */
    public UVFilterCommand( boolean included )
    {
        m_UVincluded = included;
    }

    public String getName()
    {
        return "UV Filter Command";
    }
    ...
    public SpectroEvent interpret( byte[] received )
    {
        ...
        if( m_UVincluded && received[ 0 ] != 'i' )
        {
            status.addError( "MSG_UVFILTERMISMATCH_1" );
        }
        else if( !m_UVincluded && received[ 1 ] != 'f' )
        {
            status.addError( "MSG_UVFILTERMISMATCH_2" );
        }
