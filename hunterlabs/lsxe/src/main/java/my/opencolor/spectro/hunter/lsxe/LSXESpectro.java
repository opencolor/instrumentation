...
package my.opencolor.spectro.hunter.lsxe;

import my.opencolor.spectro.hunter.common.DataControlMessage;
import my.opencolor.spectro.hunter.common.HunterCommDriver;
import my.opencolor.spectro.hunter.common.ResponseMessage;
 ...
 *
 * To this effect, the LSXESpectro has two First In First Out (FIFO)
 * queues, one for outgoing commands and one for incoming commands.
 * There is a single thread that runs continuously while the LSXESpectro
 ...
 * the LSXESpectro will throw exceptions indicating that it is busy until
 ...
 *
 * The other functions of the LSXESpectro are all event driven. Procedures
 ...
 * receive, the LSXESpectro will retrieve available data from the CommDriver
 ...
 * Otherwise, a null is returned and the LSXESpectro would guess at interpreting
 ...
 */
public class LSXESpectro
    implements Spectrophotometer, CommDriverListener, Runnable
{
    static private Logger m_Logger = Logger.getLogger( LSXESpectro.class.getName() );
    ...
    protected int m_CurrentPortPlate = 1750;

    protected String m_SerialNo;
    //Serial number of the instrument, none for the LSXE
    ...
    protected boolean whiteCalibration;
     ...
     */
    public void calibrate( int step ) throws SpectroException
    {
        try
        {
            if( step > 2 )
            ...
            m_Outgoing.putObject(
                new CalibrateCommand(
                    new Integer( step ),
                    new Integer( m_CurrentPortPlate ) ) );
        }
        catch( FifoFullException fullEx )
        {
            throw new SpectroException( "MSG_SPECTRO_BUSY" );
    ...
    public void setSettings( SpectroSettings newSettings )
    {
        synchronized( this )
        {
                ...
                catch( NumberFormatException numEx )
                {
                    ...
                    catch( CommDriverException commEx )
                    {
                        ...
                        SpectroStatus status =
                            LSXEStatus.create( "EEEEEEEEEEEEEEEEEEEE" );
            ...
            {
                m_Logger.finer( "Setting " + newSettings.getLensPosition().getName() );
                    ...
                    newSettings.getLensPosition().getName() ) )
                {
                    ...
                    m_newSettings.setLensPosition(
                        newSettings.getLensPosition() );

                    LensPosition newLensPosition =
                        m_newSettings.getLensPosition();

                    if( newLensPosition instanceof LargeAreaView )
                    {
                        m_newSettings.setAperture( new LargeAperture() );
                        m_CurrentPortPlate = 1750;
                    }
                    else if( newLensPosition instanceof MediumAreaView )
                    {
                        m_newSettings.setAperture( new MediumAperture() );
                        m_CurrentPortPlate = 1000;
                    }
                    else if( newLensPosition instanceof NormalAreaView )
                    {
                        m_newSettings.setAperture( new NormalAperture() );
                        m_CurrentPortPlate = 500;
                    }
                    else if( newLensPosition instanceof SmallAreaView )
                    {
                        m_newSettings.setAperture( new SmallAperture() );
                        m_CurrentPortPlate = 250;
                    }
                    else if( newLensPosition instanceof UltraSmallAreaView )
                    {
                        m_newSettings.setAperture( new UltraSmallAperture() );
                        m_CurrentPortPlate = 125;
                    }
                    else
                    {
                        m_CurrentPortPlate = 1000;
                    ...
                    {
                        m_Outgoing.putObject(
                            new StandardizeModeCommand( "0", String.valueOf( m_CurrentPortPlate ) ) );
                    }
                    catch( FifoFullException fullEx )
                    {
    ...
    protected void interpret( String message )
    {
        ...
        if( !response.verifyChecksum() )
        {
            ...
            SpectroStatus status = LSXEStatus.create( "E" );
        ...
        if( !m_Incoming.isEmpty() )
        {
            ...
            //format
            SpectroEvent evt =
                cmd.interpret( response.getParameter().getBytes() );
            ...
            if( evt != null )
            {
                ...
                if( cmd instanceof MeasureCommand )
                {
                    ...
                    ( (LSXEReading) evt.getReading() ).setSettings( m_Settings );

                    //DEBUG
                    if( evt.getReading().getSettings() != null )
                    {
                        m_Logger.finer( "Settings set to reading" );
                    }
                    else
                    {
                        m_Logger.finer( "Settings is null" );
                    }
                ...
                {
                    m_Logger.finer( "Calibration command " );
                    m_Incoming.removeNextObject(); //Remove from fifo
                    m_Logger.finer( "Removing " + cmd.getName() + " from Incoming" );
        ...
        {
            m_Logger.finer( "No command in incoming" );
        }
    ...
    protected void guessInterpret( String message )
    {
        ...
        case 'M':
            ...
            {
            case '0':
                m_Logger.finer( "Received Motor Speed Profile response" );
                break;
            case '1':
                m_Logger.finer( "Received Motor Control response" );
                break;
            case '3':
                m_Logger.finer( "Received Adjustable End-User Positions response" );
                break;
            case '4':
                m_Logger.finer( "Received Disable Device response" );
                break;
            default:
                m_Logger.finer( "Unknown Response" );
            }
            break;
        case 'Q':
            m_Logger.finer( "Received Flash Lamp Settings response" );
            break;
        case 'V':
            m_Logger.finer( "Received UV Control Parameters response" );
            break;
        case 'A':
            m_Logger.finer( "Received Sensor Parameters response" );
            break;
        case 'P':
            m_Logger.finer( "Received Port Plate Settings response" );
            break;
        case 'T':
            m_Logger.finer( "Received Production Self Test response" );
            break;
        case 'F':
            m_Logger.finer( "Received Select Standardization Mode" );
            m_Logger.finer( "Error : Should have been interpreted by command" );
            break;
        case 'W':
            m_Logger.finer( "Received Instrument Tile Values response" );
            break;
        case 'I':
            m_Logger.finer( "Received Standardize response" );
            m_Logger.finer( "Error : Should have been interpreted by command" );
            break;
        case 'H':
            m_Logger.finer( "Recevied Photometric Data response" );
            m_Logger.finer( "Error : Should have been interpreted by command" );
            break;
        case 'G':
            m_Logger.finer( "Received Read Ganz Whiteness response" );
            break;
        case 'U':
            m_Logger.finer( "Received UV Calibration Procedure response" );
            break;
        case 'Z':
            m_Logger.finer( "Received Error Codes response" );
            break;
        case 'X':
            m_Logger.finer( "Received Control Status response" );
            break;
        case 'D':
            m_Logger.finer( "Received Calibrate Devices or Status of Sensor response" );
            break;
        case 'K':
            m_Logger.finer( "Received Macro Button response" );
            break;
        case 'Y':
            m_Logger.finer( "Received Serial Number response" );
            break;
        case '#':
            m_Logger.finer( "Received Capability response" );
            break;
        }
    }
    ...
    public void received( CommDriverEvent evt )
    {
        ...
        if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_RECEIVED )
        {
            ...
            m_Logger.finer( "Received : " + m_Received + " (" + m_Received.length() + ")" );

            for( int i = 0; i < m_Received.length(); i++ )
            {
                m_Logger.finer( "Recv : " + (int) m_Received.charAt( i ) );
            }

            //Parse received message
            ResponseMessage response =
                new ResponseMessage( m_Received.toString() );

            //Determine if the message is complete
            m_Logger.finer( "Received : " + m_Received );
            m_Logger.finer( "Response : " + response.toString() );

            //Message is complete.
            //Interpret command now

            m_Logger.finer( response.toString() );
        ...
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_TIMEOUT )
        {
            ...
            SpectroStatus status = LSXEStatus.create( "EEEEEEEEEEEEEEEEEEEE" );
        ...
        else if( evt.getStatus() == CommDriverEvent.COMM_DRIVER_SENT )
        {
            ...
            m_Logger.finer( "LSXE Spectro : Comm Driver Sent" );
        }
        else
        {
    ...
    public void run()
    {
        while( running )
        {
            ...
            if( !m_Incoming.isFull() )
            {
                ...
                DataControlMessage message =
                    new DataControlMessage( 1, cmd.construct() );

                m_Logger.finer( "Message constucted > " + message + " (" + message.toString().length() + ")" );
