...
package my.opencolor.spectro.hunter.cqxe;
...
public class CQXESpectro implements Spectrophotometer, CommDriverListener, Runnable
{
		...
		synchronized (this)
		{
			...
			if (m_CommDriver == null)
			{
				...
				try
				{
					...
					m_OpStatus = OPERATIONAL_STATUS_IDLE;
					notifyStatusChange(new SpectroEvent(this));
					m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Comm Settings complete... return");
					return;
				}
				...
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Should not reach this return in set settings");
				return;
			}
			...
			{
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Setting " + newSettings.getLensPosition().getName());
				if (m_Settings.getLensPosition() == null || !m_Settings.getLensPosition().getName().equals(newSettings.getLensPosition().getName()))
				{
					if (m_Settings.getLensPosition() != null)
					{
						m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Current Lens Position " + m_Settings.getLensPosition().getName());
						m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "New Lens Position " + newSettings.getLensPosition().getName());
					}
					else
					{
						m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "New Lens Position " + newSettings.getLensPosition().getName());
				...
				else
				{
					...
					m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Current Settings " + m_Settings.getLensPosition().getName());
					m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "New     Settings " + newSettings.getLensPosition().getName());
				}
			}
			...
			//	lijen
				...
				try
				{
					...
					m_Outgoing.putObject(new StandardizeModeCommand(uvSettings, String.valueOf(m_CurrentPortPlate), m_newSettings.getSpecular()));
				}
				catch (FifoFullException fullEx)
				{			
					m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Wait for fifo to be cleared.");
					m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Command " + ((SpectroCommand) m_Outgoing.getNextObject()).getName());
				}
			...
			m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Lens     : " + newSettings.getLensPosition());
			m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "LightFil : " + newSettings.getLightFilter());
	 ...
	 *  @throws NotSupportectExcetpion This is not supported by the LSXE Spectros
	 */
	public void setStandard(int position, SpectroReading reading) throws SpectroException
	{
		//Not supported
	...
	protected void interpret(String message)
	{
		...
		if (!m_Incoming.isEmpty())
		{
			...
			m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Interpreting as a " + cmd.getName() + " command");
			...
			if (evt != null)
			{
				//Stop timeout timer
				m_CommDriver.cancelRespondTimeout();
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Cancelling timeout at " + System.currentTimeMillis());
				...
				if (cmd instanceof MeasureCommand)
				{
					...
					m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Removing " + cmd.getName() + " from Incoming");
	...
	protected void guessInterpret(String message)
	{
		...
		switch (message.charAt(0))
		{
			case 'M' :
				...
				{
					case '0' :
						m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received Motor Speed Profile response");
						break;
					case '1' :
						m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received Motor Control response");
						break;
					case '3' :
						m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received Adjustable End-User Positions response");
						break;
					case '4' :
			...
			case 'F' :
				...
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Error : Should have been interpreted by command");
				break;
			case 'W' :
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received Instrument Tile Values response");
				break;
			case 'I' :
			...
			case 'G' :
				...
				break;
			case 'U' :
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received UV Calibration Procedure response");
				break;
			case 'Z' :
			...
			case 'X' :
				...
				break;
			case 'D' :
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received Calibrate Devices or Status of Sensor response");
				break;
			case 'K' :
	...
	public void received(CommDriverEvent evt)
	{
		...
		if (evt.getStatus() == CommDriverEvent.COMM_DRIVER_RECEIVED)
		{
			...
			m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Received : " + m_Received + " (" + m_Received.length() + ")");

			for (int i = 0; i < m_Received.length(); i++)
			{
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Recv : " + (int) m_Received.charAt(i));
			}
		...
		else if (evt.getStatus() == CommDriverEvent.COMM_DRIVER_TIMEOUT)
		{
			...
			m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Timeout received at " + System.currentTimeMillis());
	...
	public void run()
	{
		while (running)
		{
			...
			if (!m_Incoming.isFull())
			{
				...
				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Message constucted > " + message + " (" + message.toString().length() + ")");

				m_Logger.finer(Thread.currentThread().getName() + " " + m_ID + "Sending " + cmd.getName() + " at " + System.currentTimeMillis());
